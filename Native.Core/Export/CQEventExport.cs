...
namespace Native.App.Export
{
	...
	{	
		...
		{	
			return "9,com.himagerobot.csharp";	
		}	
		...
		{	
			...
			Type appDataType = typeof (AppData);	
			// 注册一个 CQApi 实例	
			AppInfo appInfo = new AppInfo ("com.himagerobot.csharp", 1, 9, "GHS机器人forC#", "1.0.0", 1, "AlexJiang", "这是一款由C#编写的GHS机器人，数据由CQ-picfinder-robot接口提供。", authCode);	
			appDataType.GetRuntimeProperty ("CQApi").GetSetMethod (true).Invoke (null, new object[] { new CQApi (appInfo) });	
			AppData.UnityContainer.RegisterInstance<CQApi> ("com.himagerobot.csharp", AppData.CQApi);	
			// 向容器注册一个 CQLog 实例	
			appDataType.GetRuntimeProperty ("CQLog").GetSetMethod (true).Invoke (null, new object[] { new CQLog (authCode) });	
			AppData.UnityContainer.RegisterInstance<CQLog> ("com.himagerobot.csharp", AppData.CQLog);	
